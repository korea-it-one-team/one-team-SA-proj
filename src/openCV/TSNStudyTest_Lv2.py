...
def train_tsn():
    ...
    for epoch in range(num_epochs):
        ...
        for batch in DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=4):
            if batch is None or any(item is None for item in batch):  # None 값 무시
                print(f"Skipping invalid batch: {batch}")
                continue

            images, labels = batch
            try:
                images, labels = images.to(device), labels.to(device) if isinstance(labels, torch.Tensor) else torch.tensor(labels).to(device)
            except Exception as e:
